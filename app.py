...
if analysis_type == "Dialogue Count":
...
elif analysis_type == "Scene & Line Analysis":
    st.header("Scene & Line Analysis")
    st.markdown("Comparison of how many scenes characters appear in vs. how many lines they speak.")

    top_n = st.slider("Number of characters to show", 5, 30, 12)
    ...
    top_n = st.slider("Number of characters to show", 5, 30, 15)
    fig = Visualizer.plot_sentiment_distribution(processed_df, top_n=top_n, show=False)
    st.pyplot(fig)

elif analysis_type == "Sentiment Evolution (Time Series)":
...
elif analysis_type == "Interaction Network Graph":
